    ...
    componentWillMount() {
        ...
        request.get(url).end((err, res) => {
            if (err) {
                ...
                this.setState({versions: res.body, selectedVersion: res.body[0]});
        ...
        return (
            <div style={styles.div}>
                {(!(this.state.versions && this.state.versions.length === 0))
                  && <DropdownList style={styles.dropDown} data={this.state.versions} value={this.state.selectedVersion} onChange={this.handleChange}/>}
            </div>
        )
    }
}
